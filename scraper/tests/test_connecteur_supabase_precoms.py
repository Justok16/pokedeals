...
OPTIONNEL et NON BLOQUANT (cf. module docstring) : modele BROADCAST (tous
les abonnes actifs recoivent toutes les alertes), erreur reseau -> log +
skip, jamais une exception qui remonte."""
...
def test_notifier_liste_vide_ne_declenche_aucun_appel():
    with patch("connecteur_supabase_precoms._lister_abonnes_actifs") as actifs_mock:
        notifier_abonnes_precoms(
...
def test_notifier_sans_abonne_actif_ne_notifie_personne():
    secrets = {
    ...
    with patch("connecteur_supabase_precoms._lister_abonnes_actifs", return_value=[]), \
         patch("connecteur_supabase_precoms._lister_abonnements_push") as push_list_mock:
        notifier_abonnes_precoms(secrets, [_precommande()])
    push_list_mock.assert_not_called()


def test_notifier_push_actif_notifie_tous_les_abonnes_actifs_abonnes_au_push():
    secrets = {
        "POKEPRECOMS_SUPABASE_URL": "https://x.supabase.co", "POKEPRECOMS_SUPABASE_SERVICE_ROLE_KEY": "k",
        "VAPID_PRIVATE_KEY": "priv", "VAPID_CLAIM_EMAIL": "a@b.com",
    }
    with patch("connecteur_supabase_precoms._lister_abonnes_actifs", return_value=["u1", "u2"]), \
        ...
        notifier_abonnes_precoms(secrets, [_precommande()])
    # Seul u1 a un abonnement push -- u2 (actif mais pas abonné push) n'est pas notifié par push.
...
def test_notifier_email_actif_notifie_tous_les_abonnes_actifs_sans_preference_desactivee():
    secrets = {
        "POKEPRECOMS_SUPABASE_URL": "https://x.supabase.co", "POKEPRECOMS_SUPABASE_SERVICE_ROLE_KEY": "k",
...
def test_notifier_deux_precommandes_meme_utilisateur_email_recherche_une_seule_fois():
    ...
    with patch("connecteur_supabase_precoms._lister_abonnes_actifs", return_value=["u1"]), \
...
def test_lister_abonnes_actifs_sans_secrets_ne_declenche_aucun_appel_reseau():
    with patch("connecteur_supabase_precoms.requests.get") as get_mock:
        result = _lister_abonnes_actifs("", "")
    assert result == []
    get_mock.assert_not_called()


def test_lister_abonnes_actifs_erreur_reseau_retourne_liste_vide():
    with patch("connecteur_supabase_precoms.requests.get", side_effect=requests.RequestException("boom")):
        result = _lister_abonnes_actifs("https://x.supabase.co", "cle")
    assert result == []


def test_lister_abonnes_actifs_succes_filtre_active_et_trialing():
    reponse = Mock()
    reponse.raise_for_status = Mock()
    reponse.json.return_value = [{"user_id": "u1"}, {"user_id": "u2"}]
    with patch("connecteur_supabase_precoms.requests.get", return_value=reponse) as get_mock:
        result = _lister_abonnes_actifs("https://x.supabase.co", "cle")
    assert result == ["u1", "u2"]
    _, kwargs = get_mock.call_args
    assert kwargs["params"]["status"] == "in.(active,trialing)"
