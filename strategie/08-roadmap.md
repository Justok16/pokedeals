# 08 — Trajectoire 12 mois et 3-5 ans

> Direction stratégique, **pas** une prévision. Aucun chiffre de cette page
> n'est garanti ; ils servent à savoir si l'on avance ou non.

## Critère de richesse (ce qu'on optimise vraiment)

On ne pilote pas le chiffre d'affaires. On pilote, dans l'ordre :

1. **Revenu récurrent** — le seul qui transforme un travail en actif ;
2. **Marge** — un revenu à 20 % de marge n'est pas un revenu ;
3. **Revenu par heure humaine** — la vraie mesure de l'automatisation ;
4. **Valeur de l'actif** (audience possédée, donnée, marque, code) ;
5. **Revendabilité** — un actif qui ne peut pas être cédé est un emploi.

Exemple concret qui tranche les arbitrages : **un business à 5 000 €/mois avec
80 % de marge et 8 h/semaine est très supérieur à un business à 10 000 €/mois
avec 1 000 € de marge et 60 h/semaine.** Le second n'est pas une entreprise,
c'est un travail mal payé avec plus de risque.
...
## Stratégie internationale (T4 au plus tôt)

Déclencheur : un concept FR stabilisé, rentable et automatisé. Pas avant —
dupliquer un concept non prouvé double le travail et les erreurs.

Méthode, dans cet ordre :
1. identifier les contenus/pages gagnants en français ;
2. comprendre **pourquoi** ils gagnent (donnée exclusive ? moment ? format ?) ;
3. vérifier que la raison **existe aussi** sur le marché anglophone — c'est
...
Progression visée : **créateur → opérateur → propriétaire d'un système →
...
Quand du revenu arrive, il se répartit selon une règle écrite d'avance, pas
selon l'envie du moment :

| Part | Destination | Justification |
|---|---|---|
| **40 %** | Conservé (trésorerie) | Un projet sans trésorerie prend de mauvaises décisions sous pression |
| **30 %** | Automatisation et fiabilité | C'est ce qui achète du temps, donc de la capacité de test |
| **20 %** | Croissance de l'actif gagnant | Uniquement sur ce dont le ROI est **mesuré**, jamais estimé |
| **10 %** | Expérimentation | Le droit de se tromper, budgété |

Aucune dépense n'est engagée sans répondre à : **quel revenu supplémentaire, ou
quelles heures économisées, en combien de temps ?** Si la réponse est
« probablement », la dépense attend.

## Scénarios à 12 mois (pour décider sans se raconter d'histoires)
...
Le scénario le plus probable n'est pas le succès. **C'est pour ça que les règles
de KILL et le coût marginal quasi nul de chaque concept comptent plus que le
choix de la « bonne niche ».**
